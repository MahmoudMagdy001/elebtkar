// ponytail: Premium list of pricing solutions with price details, popularity toggles, and edit/delete actions.
...
export default function AdminPricingPlans() {
  ...
  const { plans, loading, error, refetch, deletePlan } = usePricing();
  ...
  const handleDeleteRequest = (id, title) => {
    setModalConfig({
      ...
      type: 'confirm',
      title: 'تأكيد الحذف',
      message: `هل أنت متأكد من حذف باقة الأسعار: "${title}"؟ قد يؤثر ذلك على المشتريات والطلبات النشطة.`,
      id,
      itemTitle: title
    });
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (success) {
      setModalConfig({ 
        ...
        type: 'alert', 
        title: 'تم الحذف', 
        message: `تم حذف باقة الأسعار "${itemTitle}" بنجاح!` 
      });
    } else {
      setModalConfig({ 
        isOpen: true, 
        ...
        message: 'فشل حذف الباقة، يرجى المحاولة لاحقاً.' 
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-primary">خطط الأسعار (Pricing Plans)</h1>
          <p className="text-sm text-gray-500 font-bold mt-1">إدارة باقات وحلول الدفع المعروضة للعملاء والاشتراكات لوكالة ابتكار</p>
        </div>
        <div className="flex items-center gap-3 self-end sm:self-auto">
          <Button variant="secondary" onClick={() => navigate('/admin/pricing/new')}>
      ...
      <Card>
        <CardHeader className="justify-between">
          ...
          <span className="text-xs font-bold text-gray-400">إجمالي الباقات: {filteredPlans.length}</span>
        </CardHeader>
        ...
        <CardBody className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-400 font-bold">لا توجد باقات مطابقة للبحث.</div>
          ) : (
              ...
              <table className="w-full text-right border-collapse text-sm">
                <thead>
                  <tr className="bg-primary-50/40 border-b border-primary-100/50">
                    ...
                    <th className="p-4 font-extrabold text-primary">عنوان الباقة</th>
                    <th className="p-4 font-extrabold text-primary">السعر</th>
                    <th className="p-4 font-extrabold text-primary">الرابط (Slug)</th>
                    <th className="p-4 font-extrabold text-primary">الحالة المميزة</th>
                ...
                <tbody className="divide-y divide-primary-50">
                  {filteredPlans.map((plan) => (
                    <tr key={plan.id} className="hover:bg-primary-50/20 transition-colors">
                      <td className="p-4 font-mono font-bold text-gray-500">{plan.order_num || '-'}</td>
                      <td className="p-4 font-bold text-gray-900">
                        {plan.title}
                        {plan.subtitle && <p className="text-xs text-gray-400 font-normal mt-0.5">{plan.subtitle}</p>}
                      </td>
                      <td className="p-4 font-bold text-gray-900">
                        {plan.price} {plan.currency || 'SAR'}
                        {plan.billing_cycle && <span className="text-xs text-gray-400 font-normal font-almarai"> / {plan.billing_cycle}</span>}
                      </td>
                      <td className="p-4 font-mono text-xs text-gray-500" dir="ltr">{plan.slug || '-'}</td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-extrabold ${
                          ...
                          {plan.is_popular ? 'شائعة (Popular)' : 'عادية'}
                        </span>
                      </td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-extrabold ${
                          ...
                          {plan.is_active !== false ? 'نشط' : 'غير نشط'}
                        </span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() => handleDeleteRequest(plan.id, plan.title)}
