// ponytail: Premium list of services with order indicators, slug links, and edit/delete actions.
...
export default function AdminServices() {
  ...
  const handleDeleteRequest = (id, title) => {
    setModalConfig({
      ...
      type: 'confirm',
      title: 'تأكيد الحذف',
      message: `هل أنت متأكد من حذف الخدمة: "${title}"؟ قد يؤثر ذلك على الصفحات المعروضة للزوار.`,
      id,
      itemTitle: title
    });
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    if (success) {
      setModalConfig({ 
        ...
        type: 'alert', 
        title: 'تم الحذف', 
        message: `تم حذف الخدمة "${itemTitle}" بنجاح!` 
      });
    } else {
      setModalConfig({ 
        isOpen: true, 
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-primary">إدارة الخدمات (Services)</h1>
          <p className="text-sm text-gray-500 font-bold mt-1">إدارة الخدمات المعروضة على الصفحة الرئيسية وقسم الخدمات لوكالة ابتكار</p>
        </div>
        <div className="flex items-center gap-3 self-end sm:self-auto">
          <Button variant="secondary" onClick={() => navigate('/admin/services/new')}>
      ...
      <Card>
        <CardHeader className="justify-between">
          ...
          <span className="text-xs font-bold text-gray-400">إجمالي الخدمات: {filteredServices.length}</span>
        </CardHeader>
        ...
        <CardBody className="p-0">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-400 font-bold">لا توجد خدمات مطابقة للبحث.</div>
          ) : (
              ...
              <table className="w-full text-right border-collapse text-sm">
                <thead>
                  <tr className="bg-primary-50/40 border-b border-primary-100/50">
                    ...
                    <th className="p-4 font-extrabold text-primary">عنوان الخدمة</th>
                    <th className="p-4 font-extrabold text-primary">الوصف الفرعي</th>
                ...
                <tbody className="divide-y divide-primary-50">
                  {filteredServices.map((service) => (
                    <tr key={service.id} className="hover:bg-primary-50/20 transition-colors">
                      <td className="p-4 font-mono font-bold text-gray-500">{service.order_num || '-'}</td>
                      <td className="p-4 font-bold text-gray-900">{service.title}</td>
                      <td className="p-4 text-gray-500 font-bold max-w-xs truncate">{service.subtitle || '-'}</td>
                      <td className="p-4 font-mono text-xs text-gray-500" dir="ltr">{service.slug}</td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-extrabold ${
                          ...
                          {service.is_featured ? 'مميزة (Featured)' : 'عادية'}
                        </span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() => handleDeleteRequest(service.id, service.title)}
